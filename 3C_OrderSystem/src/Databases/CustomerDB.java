...
public class CustomerDB {
	...
	public static Customer getCustomerbyID(int custID)
		...
		try{
			String createString = "select * from " + Databases.CustomerDB.CUSTOMER_TABLE_NAME 
					  + " where CUSTOMER_ID like " + custID + ";" ;                
			stmt = sqlConn.createStatement();
	...
	public static java.util.ArrayList getAllCustomers()
		...
		try{
			...
			while (rs.next() == true)
			results.add(new Objects.Customer (rs.getInt("CUSTOMER_ID"), 
					  rs.getString("FIRST_NAME"), rs.getString("LAST_NAME"),
					  rs.getString("EMAIL"), rs.getString("PHONE"), bill, ship));  
		}catch (java.sql.SQLException e){
